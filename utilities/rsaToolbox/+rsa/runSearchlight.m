function runSearchlight(Searchlight,inFiles,outFiles,analysisName,rsaFunction,varargin)
...
while (~isDone)
    ...
    save(sprintf('temp_%s_%2.2d.mat',analysisName,b),'T');
    isCalc(j) = true;
    isDone    = all(isCalc);
    b=b+1;
end
...
for b=1:numBlocks;
    load(sprintf('temp_%s_%2.2d.mat',analysisName,b));
    ...
    b=b+1;
    fprintf('block %d out of %d done\n',b,numBlocks);    
end;
fprintf('%d total: %f\n',k,toc);

% Clean up the search lights
delete(sprintf('temp_%s*.mat',analysisName));
