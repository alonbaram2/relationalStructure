...
for i=1:length(idxVol)
    ...
    Li   = rsa.defineSearchlight_volume(Structures{idxVol(i)},exclMask,currentOpt); % Alon Baram: used to also output exclMask, but gave an error
    Li.structure    = numStruct*ones(size(Li.LI,1),1);
%     L = addstruct(L,Li); % Alon Baram: changed because don't have the
%     function addstruct and only using one structure
    L=Li;
