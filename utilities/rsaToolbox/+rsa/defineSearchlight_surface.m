...
numSurf = length(Surf); % usually - 2 (hemisphers)
numVert = 0;
for i = 1:length(Surf)
...
indxoffset  = 0;
for i=1:numSurf  % usually 2 (hemispheres)
    % Alon: for a vertex N,  N2F(N,:) will give all the faces this vertex
    % is contained in. 
...
% i.e. allcoords: 3 x Opt.linedef spacing (default = 5) x numVert
