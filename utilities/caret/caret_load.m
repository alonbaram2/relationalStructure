...
switch (type)
    ...
    % Coordinate file
    case {'coord'}
        s=strmatch('configuration_id',char(M.header));
        [dummy,M.configuration_id]=strread(M.header{s},'%s%s');
    ...
    case 'topo'
        ...
        s=strmatch('perimeter_id',char(M.header));
        [dummy,M.perimeter_id]=strread(M.header{s},'%s%s');
        if (strcmp(M.encoding,'ASCII'))
    ...
    % --------------------------------------------------------------------
    % RGB_paint
    case 'RGB_paint' % Reads right now only one-column files 
        [M,t]=load_tags(fid,M);
        s=strmatch('tag-number-of-columns',char(M.tags));
        [dummy,M.num_cols]=strread(M.tags{s},'%s%d');
        s=strmatch('tag-number-of-nodes',char(M.tags));
        ...
        for c=1:3 
            s=strmatch(['tag-scale-' color{c}],char(M.tags));
            for i=1:length(s)
                [dummy,d,x1,x2]=strread(M.tags{s(i)},'%s%d%f%f');
                M.scales{d+1}(c,1)=x1;
                M.scales{d+1}(c,2)=x2;
            end; 
        end; 
        if (strcmp(M.encoding,'ASCII'))
            ...
            M.data=fread(fid,inf,'float32');
            M.index=[1:M.num_rows]';
            M.data=reshape(M.data,M.num_cols*3,M.num_rows)';
        end;
    % --------------------------------------------------------------------
    % paint files     
    case 'paint'
    ...
    case 'borderproj'  % Indices are 1-based
        ...
        for b=1:M.numborders
            ...
            B.p=fscanf(fid,'%f',4);
            B.c=fscanf(fid,'%f',3);
            data=fscanf(fid,'%f',[8 B.numpoints]);
            data=data';
            B.vertex=data(:,1:3)+1;     % Index begins at 1 
            B.weight=data(:,5:7);
            M.Border(b)=B;
        end;
    case 'border' 
        ...
        for b=1:M.numborders
            ...
            B.numpoints=fscanf(fid,'%d',1);
            B.name=fscanf(fid,'%s',1);
            B.density=fscanf(fid,'%f',1);
            B.c=fscanf(fid,'%f',6);
            data=fscanf(fid,'%f',[6 B.numpoints]);
            data=data';
            B.data=data(:,3:5);
            M.Border(b)=B;
        end;
