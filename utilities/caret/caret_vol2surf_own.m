...
%   'exclude_thres',thres: Threshold is the maximum of the biggest cluster
%                        necessary. (i.e. 0.9 means that the voxel has to lie at least to 90% on one side
%                        to be able to count in mapping
% OUTPUT:
%   M: metric structure that can be saved using caret_save
% J.diedrichsen@ucl.ac.uk, Jan 2013

ignore_zeros=0;         % In the volume-data, should zero be set to NaN? (No for accuracy, yes for beta)
exclude_thres=0;
s=[];
column_names={};
depths=[0 0.2 0.4 0.6 0.8 1];
hold=0;         % Interpolation type
stats=@(x)nanmean(x,2);

vararginoptions(varargin,{'ignore_zeros','column_names','depths','hold','stats','exclude_thres','topo'});
